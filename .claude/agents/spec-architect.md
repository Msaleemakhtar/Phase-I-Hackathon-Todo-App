...
1. **Q1: Similarity Check** - Search existing specs to detect duplicates or related features (>80% similarity = prompt user, 50-80% = reference patterns, <50% = proceed new)
...
- Known implemented features (currently: `001-add-task`)
- Project standards: Python 3.13+, pytest, CLI interface, in-memory only

You MUST check similarity against existing features before creating new specs. You MUST validate against constitution constraints (no persistence, CLI only, Task data model, Python 3.13+).
